	...
	<div class="rounded-2xl px-8 py-8 w-full max-w-sm flex flex-col items-center">
		{#if checking}
			<div class="flex flex-col items-center w-full">
				<svg class="animate-spin h-16 w-16 bg-white mb-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
					<circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
					<path class="opacity-75" fill="currentColor"
								d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/>
				</svg>
				<div class="text-lg text-center font-medium bg-white">Checking session...</div>
			</div>
		{:else}
			{#if loading}
				<div class="flex flex-col items-center w-full mb-5">
					...
					<div class="text-lg text-center font-medium bg-white mb-2">Signing in with GitHub...</div>
					<button class="mt-2 px-5 py-1.5 bg-white border border-gray-800 text-gray-800 rounded-lg hover:bg-white/50 hover:text-white transition" on:click={cancel}>Cancel</button>
				</div>
			{/if}
			{#if !loading}
				<button
					class="w-full bg-white text-white px-4 py-3 rounded-lg font-semibold hover:bg-white/50 transition mb-2"

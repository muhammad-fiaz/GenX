...
<div class="flex h-screen bg-[#0f0f0f] text-white">
	<div
		class="fixed inset-y-0 left-0 z-30 w-72 bg-[#1a1a1a] p-4 space-y-6 transform lg:translate-x-0 lg:relative lg:inset-y-auto"
		in:fly={{ x: -300 }}
		out:fly={{ x: -300 }}
	...
	>
		<div class="flex justify-between items-center mb-6">
			<div class="font-bold text-xl">Configurations</div>
			<button
				type="button"
		...
		<ul class="space-y-4">
			<li>
				<button
					type="button"
					class="flex items-center gap-3 w-full text-left py-3 px-4 rounded-lg cursor-pointer"
		...
		<a
			href="/"
			class="absolute bottom-6 left-6 px-4 py-2 rounded-lg bg-gray-700 text-white hover:bg-gray-600 inline-block"
		>
			&larr; Back
		</a>
	</div>
	...
	<div class="flex-1 p-6 lg:ml-72">
		...
		<button
			type="button"
			class="lg:hidden mb-4 text-gray-400 hover:text-white"
		...
		<div class="bg-[#1a1a1a] p-6 rounded-lg">
			{#if $currentTab === 'models'}
				<p class="text-gray-400">This is the Models content area. Add your model-related details here.</p>
			{/if}
			{#if $currentTab === 'custom'}
				<p class="text-gray-400">This is the Custom content area. Add your custom configuration here.</p>
			{/if}
			{#if $currentTab === 'prompt'}
				<p class="text-gray-400">This is the Prompt content area. Add your prompt-related details here.</p>
			{/if}
		</div>
	</div>
</div>